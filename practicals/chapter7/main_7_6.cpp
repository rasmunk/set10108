...
int main() {
    ...
    auto program = load_program("kernel_matrix_multi.cl", context, devices[0], devices.size());

    // Create the kernel object
    auto kernel = clCreateKernel(program, "simply_multiply", &status);

    // Number of elements and size of buffer on GPU
    const unsigned int elements = 256;
    const unsigned int data_size = (sizeof(float) * elements * elements);

    // Host data - stored in main memory
    array <array<float, elements>, elements> A;
    array <array<float, elements>, elements> B;
    array <array<float, elements>, elements> C;
    ...
    status = clSetKernelArg(kernel, 0, sizeof(cl_mem), &buffer_C);
    status |= clSetKernelArg(kernel, 1, sizeof(int), &elements);
    status |= clSetKernelArg(kernel, 2, sizeof(int), &elements);
    status |= clSetKernelArg(kernel, 3, sizeof(int), &elements);
    status |= clSetKernelArg(kernel, 4, sizeof(int), &elements);
    status |= clSetKernelArg(kernel, 5, sizeof(cl_mem), &buffer_A);
    status |= clSetKernelArg(kernel, 6, sizeof(cl_mem), &buffer_B);

    // Configure the work dimensions (Remember GPU works in cubes -> x,y,z dimensions)
    // 1 dimension can be created -> 1D of elements
    array<size_t, 2> global_work_size = {elements, elements};

    // Enqueue the kernel for execution // RUNS
    // cmd_queue -> command queue -> enqueues the work
    // kernel -> the kernel we are executing
    // global_work_size.data() -> number of elements per dimension
    status = clEnqueueNDRangeKernel(cmd_queue, kernel, 2, nullptr, global_work_size.data(), nullptr, 0, nullptr,
                                    nullptr);

    // Retrieve results
    // Read the output uffer from the GPU to main memory
